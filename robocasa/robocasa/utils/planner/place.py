...
class PlaceToPlanner:
    def __init__(self, env, obs, extra_para, id=0):
        ...
        # guess what object the gripper is holding
        self.obj_hold = None
        for name, obj in env.objects.items():
            if env.check_contact(obj, env.robots[self.id].gripper["right"]):
                self.obj_hold = name
                break
        ...
        # handle different extra_para types
        ...
        if extra_para in object_keys:
            ...
            # "place to container", container is in microwave
            if self.fxtr_str == 'microwave':
                ...
                self.placement_mat = target_eef_wrt_world_mat # used in place to microwave
            
            # "place to container", container is in cabinet
            elif 'cabinet' in self.fxtr_str:
                self.placement_pos = obs[self.obj_str + '_pos'] + np.array([0, 0, 0.12])
                ...
                self.placement_mat = target_eef_wrt_world_mat # used in place to cabinet
        ...
        # if extra_para is fixture keys, place to fixture surface
            ...
            if self.fxtr_str == 'microwave':
                ...
                # find the object in the microwave
                for obj_cfg in env.object_cfgs:
                    if 'fixture' in obj_cfg['placement'] and type(obj_cfg['placement']['fixture']).__name__.lower() == "microwave":
                        self.obj_str = obj_cfg['name']
                        break
                self.placement_pos = obs[self.obj_str + '_pos'] + np.array([0, 0, 0.12])
            ...
            # "place to stove", front_right burner for default
            elif self.fxtr_str == 'stove':
                base_ori = T.mat2euler(T.quat2mat(obs[f'robot{self.id}_base_quat']))[2]
                fixture_pos = self.fixture.pos
                burner_pos = fixture_pos - np.array([np.cos(base_ori), np.sin(base_ori), 0]) * 0.25 # front
                burner_pos = burner_pos + np.array([np.sin(base_ori), -np.cos(base_ori), 0]) * 0.25 # front_right
                burner_pos[2] = 1.0 # assume stove burner height is 1.0
                self.placement_pos = burner_pos
                self.above_placement_pos = self.placement_pos + np.array([0, 0, 0.10]) # move above the fixture first
    ...
    def get_control(self, env=None, obs=None):
        ...
        if (self.fxtr_str == 'microwave') or ('cabinet' in self.fxtr_str):
            return self.get_control_top(env, obs)
    ...
    def get_control_top(self, env=None, obs=None):
        """
