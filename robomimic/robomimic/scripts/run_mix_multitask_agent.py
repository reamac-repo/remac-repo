...
def run_mix_rollout_multitask_agent(
        env, 
        horizon,
        use_goals=False,
        render=False,
    ...
    if log_path == None:
        ...
        folder_path = os.path.join(BASE_PATH, "record", task_name, "agent-" + time_str)
...
""".strip()
    ...
    view = 2
    task_name = env.env.env.__class__.__name__
    agent = MixAgent(goal, env_info, view=view, task_name=task_name)
...
def run_mix_multitask_agent(args):
    # some arg checking
    ...
    for env, horizon in zip(envs, horizon_list):
        ...
        for ep_i in iterator:
            ...
            try:
                rollout_info = run_mix_rollout_multitask_agent(
                    env=env,
                    horizon=horizon,
                    render=render,
                    use_goals=use_goals,
