...
class PickUpPlanner:
    def __init__(self, env, obs, extra_para, id=0):
        ...
        # record the initial eef position and orientation
        ...
        if self.obj_str == "pan":
            ...
            # get basic position and orientation information
            ori_pos = obs[self.obj_str + '_pos']
            pan_radius = 0.15
            obj_ori = T.mat2euler(T.quat2mat(obs[self.obj_str + '_quat']))[2] + np.pi/2 # match handle direction

            # get the grasp and above position
            grasp_pos = ori_pos - np.array([np.cos(obj_ori), np.sin(obj_ori), 0]) * pan_radius
            self.obj_pos = grasp_pos
            self.above_obj_pos = self.obj_pos + np.array([0, 0, 0.15])
        ...
        # "pick up bowl"
        elif self.obj_str == "bowl":

            # get basic position and orientation information
            center_pos = obs[self.obj_str + '_pos']
            bowl_radius = 0.1
            obj_wrt_world_mat = T.quat2mat(obs[self.obj_str + '_quat'])
            bowl_ori = np.arctan2(obj_wrt_world_mat[1, 0], obj_wrt_world_mat[0, 0])

            # calculate two possible grasp positions
            grasp_pos1 = center_pos + np.array([np.cos(bowl_ori), np.sin(bowl_ori), 0]) * bowl_radius
            grasp_pos2 = center_pos - np.array([np.cos(bowl_ori), np.sin(bowl_ori), 0]) * bowl_radius
            eef_pos = obs[f'robot{self.id}_eef_pos']

            # use the closer position to grasp
            dist1 = np.linalg.norm(grasp_pos1 - eef_pos)
            dist2 = np.linalg.norm(grasp_pos2 - eef_pos)
            grasp_pos = grasp_pos1 if dist1 <= dist2 else grasp_pos2
            grasp_pos += np.array([0, 0, 0.025]) # move up a little bit
        ...
        # handle defalt objects like spherical or stick-shaped object
        else:
